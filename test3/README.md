...
根据订单日期进行范围分区。不同的年份的数据存储在不同的分区中，将2018年以前的数据存入users中，将2018年到2021年的数据存入USERS02中，将2021到2022的数据存入USERS03中

##### 1.使用system用户给自己的账号(yyt)分配上述分区的使用权限

![1](./1.png)

#### 2.在sqldevlop中使用自己的用户连接数据库

![2](./2.png)

#### 3.创建表 ORDERS 根据订单日期进行范围分区。不同的年份的数据存储在不同的分区中
...
在执行查询计划的过程中，rows代表预估操作返回的记录条数，Bytes代表预估操作返回的记录字节数，Cost(%CPU)代表预估操作所需的开销。在括号中列出了CPU开销的百分比。这些值是执行计划计算出来的。一般情况下都是根据Cost来分析查询语句的效率和性能。
Pstart和Pstop代表的是访问的第一个和最后一个分区，两条查询语句都是对同一分区进行的查询。
可以明显的感受出，在经过分区创建之后，不管是单表查询还是两张表一起联合查询，所耗费的时间都很少，cost不大，效率非常的高。